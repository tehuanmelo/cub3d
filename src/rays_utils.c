...
t_ray_direction get_ray_direction(float ray_angle)
{
    t_ray_direction ray_direction;
     ...
     ray_direction.is_ray_facing_right = ray_angle < 0.5 * PI || ray_angle > 1.5 * PI;
...
void find_colision(t_data *data, t_colision *colision, bool direction)
{
    ...
    if (colision->found_wall_hit)
        colision->hit_distance = get_hit_distance(data->player.x, data->player.y, colision->wall_hit_x, colision->wall_hit_y);
}